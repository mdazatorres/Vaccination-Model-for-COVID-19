...
def Initday(clave, ZMs):
    init_trans = 3
    init0 = ZMs[clave][2] + timedelta(days=init_trans)
...
def output_vaccination(N, solns_plain_v, samples, inter_index, params, baseline):
    ...
    if baseline==True:
        ...
        samples_new[:, 1] = X0[:, 1]  # V1
        samples_new[:, 2] = X0[:, 2]  # V2
        samples_new[:, 3:fm.q] = X0[:, 3:fm.q]  # E-U-O-R-D
    ...
    for index, k in enumerate(samples_new):
        ...
        for i, sl in enumerate(tmp):  # cumulative solution1s
...
def PlotVacc(init, size_window, solns, ty, ax, q, color, label, right_axis, label_cases, data,
    ...
    if label:
        ax.plot(days, sv[:, 5], '-', linewidth=2, color=color, label=r"$\omega$=" + str(value))
    elif baseline:
...
def Plot_scn1(workdir='./../', save=True):
    close('all')
    q = [10, 25, 50, 75, 90]
    loc = 1
    ...
    for i in range(len(rates_vac)):
        samples_new, solns_v, solns_plain_v = output_vaccination(N=N, solns_plain_v=solns_plain, samples=samples, inter_index=inter_index,
                                                                 params=[lamda_v1*rates_vac[i], lamda_v2*rates_vac[i], omega_b, beta],baseline=True) # baseline model
    ...
    for i in range(len(Omega)):
        _, solns_v1, solns_plain_v1 = output_vaccination(N=N, solns_plain_v=solns_plain_v, samples=samples_new,
                                                     inter_index=inter_index, params=[lamda_v1, lamda_v2, Omega[i], beta], baseline=False)
        Solns_v.append(solns_v1)

    #--------------------- Incidence---------------------
    fig, ax = subplots(num=1, figsize=(13, 7))
    init = init0 + timedelta(days=(inter_index))
    PlotVacc(init=init, size_window=pred, solns=Solns_v[0], ty=0, ax=ax, q=q,
             color=colors[0], label=False, right_axis=False, label_cases=False,
             data=data, show_data=False, value=Omega[0], baseline=True, label_vac=None)
    for i in range(len(rates_vac)):
        PlotVacc(init=init0, size_window=inter_index, solns=soln_rates[i], ty=0, ax=ax, q=q,
                  color=colors_rates[i], label=False, right_axis=label_cases[i], label_cases=label_cases[i],
                  data=data,show_data=label_cases[i], value=omega_b, baseline=False, label_vac=label_vac[i])
    for i in range(len(Omega)-1):
        i=i+1
        PlotVacc(init=init, size_window=pred, solns=Solns_v[i], ty=0, ax=ax, q=q,
                  color=colors[i], label=True, right_axis=False, label_cases=False, data=data, show_data=False, value=Omega[i],baseline=False, label_vac=None)
    fig.tight_layout()
    if save == True:
        fig.savefig("%s%s_scn1_I.png" % (workdir + 'figures/', out_fnam))
    ...
    for i in range(len(rates_vac)):
        PlotVacc(init=init0, size_window=inter_index, solns=soln_rates[i], ty=1, ax=ax, q=q, color=colors_rates[i],
                label=False, right_axis=label_cases[i], label_cases=label_cases[i], data=data, show_data=label_cases[i], value=omega_b,
                    baseline=False, label_vac=label_vac[i])

    for i in range(len(Omega)-1):
        i=i+1
        PlotVacc(init=init, size_window=pred, solns=Solns_v[i], ty=1, ax=ax,  q=q, color=colors[i], label=True, right_axis=True, label_cases=False, data=data, show_data=False,
                    value=Omega[i], baseline=False, label_vac=None)
    fig.tight_layout()
    if save == True:
        fig.savefig("%s%s_scn1_D.png" % (workdir + 'figures/', out_fnam))
    return ax
...
#--- Uncomment the following lines to plot Figure 2 a-b
